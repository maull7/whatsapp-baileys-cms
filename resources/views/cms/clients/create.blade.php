...
<div class="mb-6">
    ...
    <p class="text-slate-600 mt-1">Setiap client punya API key untuk proyek backend WhatsApp (Node.js) di luar direktori ini.</p>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-xl">
    <form action="{{ route('cms.clients.store') }}" method="POST">
        ...
        <div class="space-y-4">
            ...
            <div>
                <label for="api_key" class="block text-sm font-medium text-slate-700 mb-1">API Key (kosongkan untuk auto-generate)</label>
                <input type="text" name="api_key" id="api_key" value="{{ old('api_key') }}" placeholder="Opsional"
            ...
            <div>
                <label for="api_base_url" class="block text-sm font-medium text-slate-700 mb-1">API Base URL (backend Node.js)</label>
                <input type="url" name="api_base_url" id="api_base_url" value="{{ old('api_base_url') }}" placeholder="https://api.example.com"
                    class="w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                @error('api_base_url')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-slate-500">Kosongkan untuk memakai WHATSAPP_API_BASE_URL di .env</p>
            </div>
        </div>
