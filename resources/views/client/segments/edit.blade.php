<x-client-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Segment</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                ...
                @endif
                <form action="{{ route('client.segments.update', $segment) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-4">
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama Segment *</label>
                            <input type="text" name="name" id="name" value="{{ old('name', $segment->name) }}" required
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100">{{ old('description', $segment->description) }}</textarea>
                        </div>
